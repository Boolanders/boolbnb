...
function placesCreateAutocomplete() {
  // funzione che crea l'autocomplete nella ricerca. si basa su un servizio di algolia
  var placesCreateAutocomplete = places({
    appId: 'pl9FR7QVJTYP',
    // questi sono chiavi che ti danno alla registrazione al sito. Limite di 10.000 ricerche al mese poi si paga.
  ...
  placesCreateAutocomplete.on('change', function (e) {
    // funzione che si attiva ad ogni change dell'input su cui abbiamo fatto l'autocomplete
...
function checkNotImagesInput(value) {
  ...
  if ($(value).is('#address-input')) {
    // siccome algolia crea in automatico l'autocomplete ma cambia un po' l'html creo due target diversi. Il target sarà comunque uno span che cambia colore se l'input è valido o no. value è l'oggetto this che ci arriva dalla funzione precedente
  ...
  if ($(value).val() == 0) {
    // se l'input è vuoto assegno colore grigio
    ...
    if (!val) {
      // input non valido coloro di rosso
      target.removeClass('text-success text-muted');
      target.addClass('text-danger');
    } else {
      // input valido coloro di verde
...
function checkImages(value) {
  ...
  if (value.files.length == 0) {
    // value è il this che ci arriva della funzione precedente. se la lenght è 0 vuol dire che non contiene file. Quindi ripristino il colore grigio dello span. e riattivo il bottone submit
    target.removeClass('text-success text-danger');
    target.addClass('text-muted');
    $('#create-submit').prop("disabled", false);
  } else if (value.files.length > 5) {
    // se ci sono più di 5 file coloro di rosso lo span e disattivo il bottone submit
    target.removeClass('text-success text-muted');
    target.addClass('text-danger');
    ...
    var wrongType = false; // ATTENZIONE: input file fornisce in un oggetto una serie di dati tra cui il tipo di file e le dimensioni. L'array dei file è in this.files (in questo caso value.files). Ciclo su tutti i files
